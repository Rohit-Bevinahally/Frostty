...
enum SplitPathBranch: Codable, Equatable, Sendable {
    case first
    case second
}
...
struct SplitTree: Codable, Equatable, Sendable {
    ...
    func insert(at leafID: UUID, toward direction: SpatialDirection, newID: UUID = UUID()) -> (tree: SplitTree, newLeafID: UUID) {
    ...
    private static func insertNode(
        ...
        case .leaf(let id):
            if id == targetID {
                ...
                let first = insertsBefore ? new : existing
                let second = insertsBefore ? existing : new
                return .split(SplitData(direction: direction, ratio: 0.5, first: first, second: second))
            }
            return node

        case .split(let data):
            let newFirst = insertNode(
                in: data.first,
                at: targetID,
                newID: newID,
                direction: direction,
    ...
    func resize(node leafID: UUID, by amount: Double, toward direction: SpatialDirection, bounds: CGSize, minSize: CGFloat) -> SplitTree {
        guard let root else { return self }

        let (newRoot, handled) = Self.resizeInNode(
            root,
            targetLeafID: leafID,
            amount: amount,
            toward: direction,
            bounds: CGRect(origin: .zero, size: bounds),
            minSize: minSize
        )
        guard handled, let newRoot else {
            return self
        }
    ...
    func settingRatio(at path: [SplitPathBranch], to ratio: Double, in splitSize: CGSize, minSize: CGFloat) -> SplitTree {
        guard let root else { return self }
        guard let newRoot = Self.settingRatio(
            in: root,
    ...
    private static func resizeInNode(
        ...
        targetLeafID: UUID,
        amount: Double,
        toward direction: SpatialDirection,
        bounds: CGRect,
        minSize: CGFloat
    ) -> (SplitNode?, Bool) {
        guard case .split(let data) = node else { return (node, false) }

        let firstContains = containsLeaf(data.first, id: targetLeafID)
        let secondContains = containsLeaf(data.second, id: targetLeafID)

        guard firstContains || secondContains else { return (node, false) }

        let (firstBounds, secondBounds) = splitBounds(bounds, direction: data.direction, ratio: data.ratio)

        if let ratioDeltaSign = resizeRatioDeltaSign(
            for: direction,
            splitDirection: data.direction
        ) {
            let axisSize = data.direction == .horizontal ? bounds.width : bounds.height
            guard axisSize > 0 else { return (node, false) }

            let ratioChange = (amount / Double(axisSize)) * ratioDeltaSign
            let minRatio = Double(minSize) / Double(axisSize)
            let newRatio = data.ratio + ratioChange
        ...
        if firstContains {
            let (newFirst, handled) = resizeInNode(
                data.first,
        ...
        if secondContains {
            let (newSecond, handled) = resizeInNode(
                data.second,
                targetLeafID: targetLeafID,
                amount: amount,
                toward: direction,
    ...
    private static func resizeRatioDeltaSign(
        for direction: SpatialDirection,
        splitDirection: SplitDirection
    ) -> Double? {
        switch direction {
        case .left:
            guard splitDirection == .horizontal else { return nil }
            return -1
        case .right:
            guard splitDirection == .horizontal else { return nil }
            return 1
        case .up:
            guard splitDirection == .vertical else { return nil }
            return -1
        case .down:
            guard splitDirection == .vertical else { return nil }
            return 1
    ...
    private static func settingRatio(
        in node: SplitNode,
        path: ArraySlice<SplitPathBranch>,
        ratio: Double,
        splitSize: CGSize,
        minSize: CGFloat
    ) -> SplitNode? {
        ...
        guard let branch = path.first else {
            let axisSize = data.direction == .horizontal ? splitSize.width : splitSize.height
            guard axisSize > 0 else { return node }

            let minRatio = Double(minSize) / Double(axisSize)
            let clampedRatio = SplitData.clampRatio(max(minRatio, min(1 - minRatio, ratio)))
            return .split(SplitData(
                direction: data.direction,
                ratio: clampedRatio,
                first: data.first,
        ...
        switch branch {
        case .first:
            guard let newFirst = settingRatio(
                in: data.first,
                path: path.dropFirst(),
                ratio: ratio,
                splitSize: splitSize,
        ...
        case .second:
            guard let newSecond = settingRatio(
                in: data.second,
                path: path.dropFirst(),
                ratio: ratio,
                splitSize: splitSize,
