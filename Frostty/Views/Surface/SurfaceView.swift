...
class SurfaceView: NSView, ObservableObject {
    ...
    /// The last known mouse location inside the surface view, in the view's local coordinates.
    ...
    /// Called when the pointer enters, moves within, or exits the surface view.
    var mouseLocationUpdateHandler: ((CGPoint?) -> Void)?
    ...
    override func becomeFirstResponder() -> Bool {
        ...
        if result {
            focusDidChange(true)
            NotificationCenter.default.post(name: .frosttySurfaceDidFocus, object: self)
        }
        return result
    }
    ...
    func updateMouseVisibility(hidden: Bool) {
        cursorVisible = !hidden
        NSCursor.setHiddenUntilMouseMoves(hidden)
    }

    private func currentMouseCursor(for shape: ghostty_action_mouse_shape_e) -> NSCursor {
        switch shape {
    ...
    private func applyCursorShape(_ shape: ghostty_action_mouse_shape_e) {
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        // Let the window's ShortcutManager have first priority over ghostty bindings.
        // This prevents Cmd+1..9 from being claimed by ghostty's goto_tab when we want workspace switching.
        if let frosttyWindow = window as? FrosttyWindow,
           let manager = frosttyWindow.shortcutManager,
           manager.shouldIntercept(event: event, firstResponder: self) {
            // Don't consume here — let sendEvent handle it via the normal path.
            return false
        }
    ...
    override func mouseEntered(with event: NSEvent) {
        ...
        mouseLocationUpdateHandler?(pos)
        let mods = EventTranslator.translateModifiers(event.modifierFlags)
    ...
    override func mouseExited(with event: NSEvent) {
        ...
        mouseLocationUpdateHandler?(nil)
        let mods = EventTranslator.translateModifiers(event.modifierFlags)
