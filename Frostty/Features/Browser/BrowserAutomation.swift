...
enum BrowserAutomation {
    ...
    static func snapshot(maxDepth: Int = 12, maxElements: Int = 500, maxTextLength: Int = 80) -> String {
        ...
        function walk(el, depth) {
            ...
            count++;
            const ref = 'e' + count;
            el.setAttribute('data-frostty-ref', ref);
    ...
    static func click(selector: String) -> String {
        let escaped = escapeJS(resolveSelector(selector))
        return wrap("""
    ...
    static func fill(selector: String, value: String) -> String {
        let escapedSelector = escapeJS(resolveSelector(selector))
    ...
    static func wait(selector: String?, text: String?, url: String?, timeout: Int) -> String {
        ...
        if let selector {
            condition = "document.querySelector('\(escapeJS(resolveSelector(selector)))')"
        } else if let text {
            condition = "document.body && document.body.innerText.includes('\(escapeJS(text))')"
        } else if let url {
            condition = "location.href.includes('\(escapeJS(url))')"
        } else {
            condition = "true"
        }
    ...
    static func clearRefs() -> String {
        wrap("""
        document.querySelectorAll('[data-frostty-ref]').forEach(el => {
            el.removeAttribute('data-frostty-ref');
        });
    ...
    static func getAttribute(selector: String, attribute: String) -> String {
        let escapedSelector = escapeJS(resolveSelector(selector))
        let escapedAttribute = escapeJS(attribute)
        return wrap("""
        const el = document.querySelector('\(escapedSelector)');
        if (!el) throw new Error('Element not found: \(escapedSelector)');
        const val = el.getAttribute('\(escapedAttribute)');
    ...
    static func scroll(direction: String, amount: Int, selector: String? = nil) -> String {
        ...
        case "right":
            ...
            dy = "0"
        default:
            preconditionFailure("Invalid direction '\(direction)'")
        }
    ...
    static func parseResponse(_ jsonString: String) -> BrowserAutomationResponse {
        ...
        if let string = json["value"] as? String {
            value = string
        } else if let number = json["value"] as? NSNumber {
            value = number.stringValue
        } else if json["value"] is NSNull || json["value"] == nil {
            value = nil
        } else if let rawValue = json["value"],
                  let encoded = try? JSONSerialization.data(withJSONObject: rawValue),
